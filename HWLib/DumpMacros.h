...
#define nd_arg(i,x) {if(Trace)_c_. Write("[" STRING(i) "] " + nd(x) + "\n");}
#define d(x) {if(Trace)_c_. Write(nd(x) + "\n");}
#define dd(x) {if(Trace)_c_. Write(x + "\n");}

#define FILE_LINE_FUNCTION HWLib::String::FilePosition(__FILE__,__LINE__,0,__FUNCTION__)
#define d_here {if(Trace) _c_. Write(FILE_LINE_FUNCTION);}
#define writeLineFilePosition  _c_. WriteLine(FILE_LINE_FUNCTION)
#define srcargdump(x) {if(Trace)_c_. Write(nd(x)+"\n").fsrcprint();}
#define dumpreturn(x) {if(Trace) \
    _c_. IndentLevel--; \
    _c_. Write(HWLib::String("return ") + nd(x) + "\n"); \
    return; }
